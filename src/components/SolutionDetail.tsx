...
import { ArrowLeft, ArrowRight, Bookmark, ChevronLeft, CircleCheck, Copy, Share2, Timer } from 'lucide-react';
...
export default function SolutionDetail() {
  ...
  }, [solution?.id, addToRecentlyViewed]);
  ...
  const hasCommandSyntax = (text: string): boolean => {
    ...
    return /^\s*(sudo|npm|yarn|git|cd|ls|mkdir|python|java|gcc|apt|brew)\s+/.test(text) || 
  ...
  const formatDescription = (description: string) => {
    ...
    return lines.map((line, i) => {
      if (hasCommandSyntax(line)) {
        return (
          <div key={i} className="bg-gray-100 dark:bg-gray-700 p-2 my-2 rounded-md font-mono text-sm relative group">
            {line}
            <button 
  ...
  if (!solution) {
    return (
      <div className="text-center py-8">
        <h2 className="text-xl font-semibold mb-4">Solution not found</h2>
        <button 
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm dark:shadow-gray-900/10 p-6 mb-8 border dark:border-gray-700">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              {solution.difficulty && <DifficultyBadge level={solution.difficulty} />}
              <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1">
                <Timer size={16} />
                <span>{estimatedTime()}</span>
              </div>
            </div>
            <p className="text-gray-600 dark:text-gray-300">{solution.shortDescription}</p>
          </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400 mb-2">
            <span>Progress</span>
            <span>Step {currentStepIndex + 1} of {solution.steps.length}</span>
          </div>
          <div className="w-full h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
            <div 
              className="h-full bg-blue-600 dark:bg-blue-500 transition-all duration-300"
              style={{ width: `${((currentStepIndex + 1) / solution.steps.length) * 100}%` }}
            ></div>
          </div>
        </div>
        ...
        {/* Current step */}
        <div className={`border-l-4 border-blue-600 dark:border-blue-500 pl-4 py-2 ${showStepAnimation ? 'opacity-0 -translate-x-4' : 'opacity-100 translate-x-0'} transition-all duration-200`}>
